...
func (sysUserRoleDao *SysUserRoleDao) BatchUserRole(ctx context.Context, db sqly.SqlyContext, users []*models.SysUserRole) {
...
func (sysUserRoleDao *SysUserRoleDao) CountUserRoleByRoleId(ctx context.Context, db sqly.SqlyContext, ids []int64) int {
	var count = 0
	query, i, err := sqly.In("select count(*) from sys_user_role where role_id in(?)", ids)
	if err != nil {
		panic(err)
	}
...
func (sysUserRoleDao *SysUserRoleDao) DeleteUserRoleInfo(ctx context.Context, db sqly.SqlyContext, userRole *models.SysUserRole) {
