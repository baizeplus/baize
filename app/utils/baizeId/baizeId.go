...
func NewNode(node int32) error {
	if node < 0 || node > 4095 {
		return errors.New("node number must be between 0 and 4095")
	}
	n = new(Node)
	n.node = node
...
func GetId() string {
	...
	if n == nil {
		...
		buf[0] = encode[(now>>48)&0x1F]
		buf[1] = encode[(now>>43)&0x1F]
		buf[2] = encode[(now>>38)&0x1F]
		buf[3] = encode[(now>>33)&0x1F]
		buf[4] = encode[(now>>28)&0x1F]
		buf[5] = encode[(now>>23)&0x1F]
		buf[6] = encode[(now>>18)&0x1F]
		buf[7] = encode[(now>>13)&0x1F]
		buf[8] = encode[(now>>8)&0x1F]
		buf[9] = encode[(now>>3)&0x1F]
		buf[10] = encode[(byte(now<<2)|rByte[0]>>5)&0x1F]
	...
	buf[8] = encode[(now>>8)&0x1F]
	buf[9] = encode[(now>>3)&0x1F]
	buf[10] = encode[((byte(now<<2))|byte(n.node>>10))&0x1F]
